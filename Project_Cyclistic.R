...
write.csv(all_trips_df_v1, "~/Project_Cyclistic/all_trips_df_v1.csv", row.names = FALSE)


# load all_trips_df_v1 data set
all_trips_df_v1 <- read.csv("~/Project_Cyclistic/all_trips_df_v1.csv")
...
all_trips_df_v1$df_month <- str_glue("{month(ymd_hms(all_trips_df_v1$started_at), label = TRUE)}, {year(ymd_hms(all_trips_df_v1$started_at))}")
...
# use_pie_current_distrubution_of_users_pool
all_trips_df_v2 %>% 
  ...
  coord_polar("y") +
  labs(title="Current distribution of cylclistic users pool",
       subtitle = "To visualise the cyclistic users pool partitioins for members and casuals",
       caption="Data from: Nov,2021 to Oct, 2022") +
  xlab("number_of_users") + ylab("users_pool")
...
all_trips_df_v2 %>% 
  ...
  ggplot(aes(x = rideable_type, color = rideable_type)) + 
  geom_bar() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  facet_wrap(~member_casual) +
  labs(title="Comparison of cylclistic use patterns by members and casuals",
...
rbind(p_time_member, p_time_casual) %>% 
  ggplot(aes(x = rideable_type, y = ride_duration, color = rideable_type)) + 
...
portable_all_trips_v2 <- aggregate(all_trips_v2$ride_length ~ all_trips_v2$member_casual + all_trips_v2$day_of_week, FUN = mean)

write.csv(portable_all_trips_v2, file = '~/Desktop/Divvy_Exercise/avg_ride_length.csv')
